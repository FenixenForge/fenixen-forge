...
const ProductPage = () => {
  ...
  const displayCategory = category.replace(/%20/g, ' ');
  const displayName = product.name.replace(/%20/g, ' ');
  const metaDescription = product.description || `Discover details about ${displayName}.`;
  const metaImage = product.image || `/images/${displayName.toLowerCase()}-og.jpg`;
  const metaTwitterImage = product.image || `/images/${displayName.toLowerCase()}-twitter.jpg`;

  return (
    <div>
      <Head>
        <meta name="description" content={metaDescription} />
        <meta property="og:title" content={`${displayName} - ${displayCategory} - Fenixen Forge`} />
        <meta property="og:description" content={metaDescription} />
        <meta property="og:image" content={metaImage} />
        <meta name="twitter:title" content={`${displayName} - ${displayCategory} - Fenixen Forge`} />
          ...
          <div>
            ...
            <div>
              <Link 
                ...
                className={pathname === `/bots/${category}/${name}/version-history` ? 'active' : ''}
              >
